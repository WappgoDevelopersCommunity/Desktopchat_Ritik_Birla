...
class App extends React.Component {
  ...
  initSocket(url) {
    ...
    socket.on('connect', () => {
      //this.appendMessage(`Connected tooooo server ${ this.state.url }`);
  ...
  render() {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('input', { type: 'url',style: { display:"none" }, id: 'server-url', ref: 'url', value: this.props.url }),
...
class UserList extends React.Component {
  render() {
    ...
    return React.createElement(
      ...
      React.createElement(
        'div',
        { id: 'user-stats',style: {fontStyle: 'italic' } },
        users.length,
        ' users online'
      )
    );
...
class ChatArea extends React.Component {
  render() {
    ...
    return React.createElement(
      'div',
      { id: 'chat' },
      React.createElement('div', { id: 'chat-text',style: { color:"#2B2D42" }, dangerouslySetInnerHTML: { __html: text } }),
      React.createElement(
        'div',
        { id: 'chat-status-msg' },
        this.props.status
